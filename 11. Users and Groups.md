...
- `/etc/passwd`: Stores information about local users.  Each line contains seven colon separated fields: 
...
- `/etc/shadow File` Stores cryptographically hashed passwords.  Each line contains nine colon separated fields:
...
| Option | Usage |
|--------|-------|
| `-a, --append` | Use with -G to add supplementary groups without replacing |
...
| `-m, --move-home` | Move home directory (use with -d) |
...
**Creating Departmental Structure Example**
